...
import sydoruk.security.TokenManager;
import sydoruk.security.TokenPayload;
...
public class UserController {
    ...
    public ResponseEntity<String> authenticateUser(@RequestBody User user) {
        UserPojo authenticatedUser = userService.findUserByEmailAndPass(user.getEmail(), user.getPassword());
    ...
    public ResponseEntity<String>  onConflictUserEmail(DataIntegrityViolationException exception){
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ClassUtils.getShortName(exception.getClass()) + ": User with email already registered");
    }

    @ExceptionHandler
    public ResponseEntity<String> onMissingUser(NoSuchElementException exception){
    ...
    public ResponseEntity<String> onMissingUser(EmptyResultDataAccessException exception){
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ClassUtils.getShortName(exception.getClass()) + ": no one user was found");
    }
